...
const SLIDE_FOCUS: Record<SlideType, string> = {
  ...
  activity: 'Focus on the activities listed: which ones drew the most submissions and participants relative to the followers they brought in, whether a longer activity period produced proportionally more entries, and what the spread between the best and weakest activity suggests for the next one. Treat submissions and participants as the outcome measures; reach and engagement are not provided here. Where a field reads as an em dash it was never filled in — say so rather than treating it as zero.',
